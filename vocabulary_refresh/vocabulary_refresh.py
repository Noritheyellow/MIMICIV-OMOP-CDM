...
def read_params():
    ...
    try:
        ...
        # default 값으로 추정.
        params = {"step": -10, "config_file": "vocabulary_refresh.conf"}
        ...
        if not os.path.isfile(params["config_file"]):
            raise getopt.GetoptError(
                "read_params() error",
                "Config file '{f}' is not found".format(f=params["config_file"]),
            )
...
def main():
    ...
    if return_code == 0 and params["step"] in [11, 10, 0]:
        ...
        run_command = gsutil_cp_csv.format(
            source_path=config["local_athena_csv_path"],
            target_path=config["gs_athena_csv_path"],
        )
        print(run_command)
        return_code = os.system(run_command)
        print("return_code", return_code)
    ...
    if return_code == 0 and params["step"] in [12, 10, 0]:
        ...
        run_command = run_command_load.format(
            step_name="athena", config_file=params["config_file"]
        )
        print(run_command)
        return_code = os.system(run_command)
        print("return_code", return_code)
    ...
    if return_code == 0 and params["step"] in [21, 20, 0]:
        ...
        run_command = gsutil_cp_csv.format(
            source_path=config["local_mapping_csv_path"],
            target_path=config["gs_mapping_csv_path"],
        )
        print(run_command)
        return_code = os.system(run_command)
        print("return_code", return_code)
    ...
    if return_code == 0 and params["step"] in [22, 20, 0]:
        run_command = run_command_load.format(
            step_name="mapping", config_file=params["config_file"]
        )
        print(run_command)
        return_code = os.system(run_command)
        print("return_code", return_code)
    ...
    if return_code == 0 and params["step"] in [32, 30, 0]:
        run_command = run_command_bq_script.format(
            config_file=params["config_file"], script_file="vocabulary_cleanup_bq_m.sql"
        )  # remove only empty check tables
        print(run_command)
        return_code = os.system(run_command)
        print("return_code", return_code)
