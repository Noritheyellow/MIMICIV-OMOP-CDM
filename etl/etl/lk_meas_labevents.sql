...
DROP TABLE IF EXISTS mimic_omop_cdm.lk_meas_d_labitems_clean;
CREATE TABLE mimic_omop_cdm.lk_meas_d_labitems_clean AS
SELECT
    dlab.itemid                                                 AS itemid, -- for <cdm>.<source_value>
    COALESCE(dlab.loinc_code, 
    ...
    dlab.loinc_code                                             AS loinc_code, -- for the crosswalk table
    CONCAT(dlab.label, '|', dlab.fluid, '|', dlab.category)     AS source_label, -- for the crosswalk table
    CASE WHEN dlab.loinc_code IS NOT NULL
         THEN 'LOINC'
         ELSE 'mimiciv_meas_lab_loinc'
    END                                                         AS source_vocabulary_id
FROM
    mimic_omop_cdm.src_d_labitems dlab
;
...
DROP TABLE IF EXISTS mimic_omop_cdm.lk_meas_labevents_clean;
CREATE TABLE mimic_omop_cdm.lk_meas_labevents_clean AS
SELECT
    ABS(('x' || md5(gen_random_uuid()::TEXT))::bit(64)::BIGINT)  AS measurement_id,
    src.subject_id                          AS subject_id,
    src.charttime                           AS start_datetime, -- measurement_datetime,
    src.hadm_id                             AS hadm_id,
    src.itemid                              AS itemid,
    src.value                               AS value, -- value_source_value
    (REGEXP_MATCH(src.value, '^(\<=|\>=|\>|\<|=|)'))[1]::TEXT   AS value_operator,
    (REGEXP_MATCH(src.value, '[-]?[\d]+[.]?[\d]*'))[1]::REAL  AS value_number, -- assume "-0.34 etc"
    CASE WHEN TRIM(src.valueuom) <> ''
         THEN src.valueuom
         ELSE NULL
    ...
    src.trace_id            AS trace_id
FROM
    mimic_omop_cdm.src_labevents src
INNER JOIN
    mimic_omop_cdm.src_d_labitems dlab
        ON src.itemid = dlab.itemid
;
...
-- all dlab.itemid, all available concepts from LOINC and custom mapped dlab.label
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS mimic_omop_cdm.lk_meas_d_labitems_concept;
CREATE TABLE mimic_omop_cdm.lk_meas_d_labitems_concept AS
SELECT
    dlab.itemid                 AS itemid,
    dlab.source_code            AS source_code,
    dlab.loinc_code             AS loinc_code,
    ...
    vc2.standard_concept        AS target_standard_concept
FROM
    mimic_omop_cdm.lk_meas_d_labitems_clean dlab
LEFT JOIN
...
DROP TABLE IF EXISTS mimic_omop_cdm.lk_meas_labevents_hadm_id;
CREATE TABLE mimic_omop_cdm.lk_meas_labevents_hadm_id AS
SELECT
    src.trace_id                        AS event_trace_id, 
    adm.hadm_id                         AS hadm_id,
    ROW_NUMBER() OVER (
...
FROM  
    mimic_omop_cdm.lk_meas_labevents_clean src
INNER JOIN 
...
DROP TABLE IF EXISTS mimic_omop_cdm.lk_meas_labevents_mapped;
CREATE TABLE mimic_omop_cdm.lk_meas_labevents_mapped AS
SELECT
    src.measurement_id                      AS measurement_id,
    src.subject_id                          AS subject_id,
    COALESCE(src.hadm_id, hadm.hadm_id)     AS hadm_id,
...
FROM  
    mimic_omop_cdm.lk_meas_labevents_clean src
INNER JOIN 
    mimic_omop_cdm.lk_meas_d_labitems_concept labc
        ON labc.itemid = src.itemid
LEFT JOIN 
    mimic_omop_cdm.lk_meas_operator_concept opc
        ON opc.source_code = src.value_operator
LEFT JOIN 
    mimic_omop_cdm.lk_meas_unit_concept uc
        ON uc.source_code = src.valueuom
LEFT JOIN 
    mimic_omop_cdm.lk_meas_labevents_hadm_id hadm
        ON hadm.event_trace_id = src.trace_id
        AND hadm.row_num = 1
;