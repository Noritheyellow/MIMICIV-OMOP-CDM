...
DROP TABLE IF EXISTS mimic_omop_cdm.lk_meas_operator_concept;
CREATE TABLE mimic_omop_cdm.lk_meas_operator_concept AS
SELECT
    vc.concept_name     AS source_code, -- operator_name,
    vc.concept_id       AS target_concept_id -- operator_concept_id
FROM
...
CREATE TABLE mimic_omop_cdm.tmp_meas_unit AS
SELECT
    vc.concept_code                         AS concept_code,
    vc.vocabulary_id                        AS vocabulary_id,
    vc.domain_id                            AS domain_id,
...
DROP TABLE IF EXISTS mimic_omop_cdm.lk_meas_unit_concept;
CREATE TABLE mimic_omop_cdm.lk_meas_unit_concept AS
SELECT
    vc.concept_code         AS source_code,
    vc.vocabulary_id        AS source_vocabulary_id,
    vc.domain_id            AS source_domain_id,
    ...
    vc2.concept_id          AS target_concept_id
FROM
    mimic_omop_cdm.tmp_meas_unit vc
LEFT JOIN
...
DROP TABLE IF EXISTS mimic_omop_cdm.tmp_meas_unit;