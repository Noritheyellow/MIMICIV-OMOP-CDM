...
DROP TABLE IF EXISTS custom_mapping.tmp_custom_concept;

CREATE TABLE custom_mapping.tmp_custom_concept AS
SELECT
    voc.source_concept_id           AS concept_id,
    voc.concept_name                AS concept_name,
    voc.source_domain_id            AS domain_id,
    ...
    -- MIN(voc.load_row_id)            AS load_row_id
FROM
    custom_mapping.tmp_custom_mapping voc
GROUP BY
    voc.source_concept_id,
    voc.concept_name,
    voc.source_domain_id,
...
DROP TABLE IF EXISTS custom_mapping.tmp_custom_concept_relationship;

CREATE TABLE custom_mapping.tmp_custom_concept_relationship AS
SELECT
    tcr.source_concept_id               AS concept_id_1,
    CASE
        WHEN tcr.target_concept_id = 0 THEN tcr.source_concept_id
    ...
    -- tcr.load_row_id                     AS load_row_id
FROM
    custom_mapping.tmp_custom_mapping tcr
WHERE
    tcr.target_concept_id IS NOT NULL
...
DROP TABLE IF EXISTS custom_mapping.tmp_custom_vocabulary_dist;

CREATE TABLE custom_mapping.tmp_custom_vocabulary_dist AS
SELECT
    voc.source_vocabulary_id        AS source_vocabulary_id,
...
DROP TABLE IF EXISTS custom_mapping.tmp_custom_vocabulary;

CREATE TABLE custom_mapping.tmp_custom_vocabulary AS
SELECT
    voc.source_vocabulary_id        AS vocabulary_id,
    voc.source_vocabulary_id        AS vocabulary_name,
    'Odysseus generated'            AS vocabulary_reference,
    ...
    voc.load_row_id                 AS load_row_id
FROM
    custom_mapping.tmp_custom_vocabulary_dist voc
;
...
DROP TABLE IF EXISTS custom_mapping.tmp_voc_concept;

CREATE TABLE custom_mapping.tmp_voc_concept AS
SELECT *
FROM
    standardized_vocabularies.concept
WHERE
    concept_id < 2000000000
;
...
DROP TABLE IF EXISTS custom_mapping.tmp_voc_concept_relationship;

CREATE TABLE custom_mapping.tmp_voc_concept_relationship AS
SELECT vr.*
FROM
    standardized_vocabularies.concept_relationship vr
INNER JOIN
    custom_mapping.tmp_voc_concept vc1
        ON  vc1.concept_id = vr.concept_id_1
INNER JOIN
    custom_mapping.tmp_voc_concept vc2
        ON  vc2.concept_id = vr.concept_id_2
;
...
INSERT INTO custom_mapping.tmp_voc_concept
SELECT
    voc.concept_id              AS concept_id,
    voc.concept_name            AS concept_name,
    voc.domain_id               AS domain_id,
    ...
    -- when loaded not by vocabulary_refresh, raw concept tables have no load_row_id, so comment it temporarily
FROM 
    custom_mapping.tmp_custom_concept voc
-- LEFT JOIN
--     custom_mapping.concept vc
--         ON  vc.concept_id = voc.concept_id
-- WHERE
--     vc.concept_id IS NULL
;

DROP TABLE IF EXISTS standardized_vocabularies.concept;

CREATE TABLE standardized_vocabularies.concept AS
SELECT * 
FROM custom_mapping.tmp_voc_concept;

-- ----------------------------------------------------------------------
-- Add relationships to the added custom concepts
-- ----------------------------------------------------------------------

INSERT INTO custom_mapping.tmp_voc_concept_relationship
SELECT
    tcr.concept_id_1             AS concept_id_1,
    tcr.concept_id_2             AS concept_id_2,
    tcr.relationship_id          AS relationship_id,
    ...
    -- tcr.load_row_id              AS load_row_id
FROM 
    custom_mapping.tmp_custom_concept_relationship tcr
-- LEFT JOIN 
--     custom_mapping.concept_relationship vcr
--         ON  tcr.concept_id_1 = vcr.concept_id_1
--         AND tcr.concept_id_2 = vcr.concept_id_2
-- WHERE
--     vcr.concept_id_1 IS NULL
...
DROP TABLE IF EXISTS standardized_vocabularies.concept_relationship;

CREATE TABLE standardized_vocabularies.concept_relationship AS
SELECT *
FROM custom_mapping.tmp_voc_concept_relationship;

-- ----------------------------------------------------------------------
-- Re-write vocabularies to remove previous version of custom vocabularies
-- ----------------------------------------------------------------------

DROP TABLE IF EXISTS custom_mapping.tmp_voc_vocabulary;

CREATE TABLE custom_mapping.tmp_voc_vocabulary AS
SELECT *
FROM
    standardized_vocabularies.vocabulary
WHERE
    vocabulary_concept_id < 2000000000
;
...
INSERT INTO custom_mapping.tmp_voc_vocabulary
SELECT
    voc.vocabulary_id         AS vocabulary_id,
    voc.vocabulary_name       AS vocabulary_name,
    voc.vocabulary_reference  AS vocabulary_reference,
    ...
    -- voc.load_row_id             AS load_row_id
FROM 
    custom_mapping.tmp_custom_vocabulary voc
;

DROP TABLE IF EXISTS standardized_vocabularies.vocabulary;

CREATE TABLE standardized_vocabularies.vocabulary AS
SELECT *
FROM custom_mapping.tmp_voc_vocabulary;

INSERT INTO standardized_vocabularies.concept
SELECT
    vcv.vocabulary_concept_id   AS concept_id,
    vcv.vocabulary_name         AS concept_name,
    'Metadata'                  AS domain_id,
    ...
    -- NULL                        AS load_row_id
FROM 
    custom_mapping.tmp_custom_vocabulary vcv 
;
...
DROP TABLE IF EXISTS custom_mapping.tmp_custom_concept_skipped;

CREATE TABLE custom_mapping.tmp_custom_concept_skipped AS
SELECT
    tcc.*
FROM
    custom_mapping.tmp_custom_concept tcc
INNER JOIN
    standardized_vocabularies.concept vc
        ON  tcc.concept_id = vc.concept_id
        AND tcc.concept_name <> vc.concept_name
;
